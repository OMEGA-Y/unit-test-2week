...
//        champion.setName("조이");
    ...
    // 4-4. champion 객체에서 이름을 가져오는 로직이 200ms 시간 이내에 1번 실행되었는지 검증하는 로직을 작성하세요.
    @Test
...
//        verify(champion, timeout(200).atLeastOnce()).setName(anyString());
        // times를 사용하면 정확하게 실행되는 횟수를 카운트해서 검증할 수 있다.
    ...
    public void 리스트객체사이즈검증(){
        ...
        // mockService는 mock으로 annotation을 안 달았기 때문에 아래와 같이 verify하지 못한다.
//        verify(mockService, times(1)).findAllChampions();
    ...
    public void 가짜챔피언객체리턴테스트(){
        when(mockService.findByName(any(String.class))).thenReturn(new Champion("티모","탑",4));
    ...
    public void 여러개검색호출횟수검증(){
        ...
        // when으로 findAllChampions만 선언했기 때문에 findByName을 사용하면 널포인트 에러가 뜬다.
//        assertThat(mockService.findByName("애쉬").getName(),is("애쉬"));
    }
